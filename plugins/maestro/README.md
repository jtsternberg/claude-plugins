...
Decisions the human owns get a plain-language section in `/tmp/maestro/decisions-<session-id>.md`, written **before** the message that asks the question — what happened, why it matters, the options as the exact tokens to type, each with a one-letter alias (`` `ok` (k) ``) so one keystroke answers, the recommendation, what happens next either way. A `Next for you:` that asks for a decision carries the answer tokens, a clause naming the subject, and the ledger path with its anchor as a separate token (`… .md - #decide-92`) — a path fused to an anchor opens nothing — because `still owe "fold" or "defer"` is unanswerable to a human who left four hours ago. A decision bundling numbered recommendations also accepts `N: <change>`, overriding item N alone. Answered decisions move to a `Decided` list with the date and the answer, which is both the audit trail and the decisions half of a handoff.
