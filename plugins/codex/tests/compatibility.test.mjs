...
test('explicit-only inventory has mirrored Codex policy and risk classification', () => {
	...
	// invocation contract has to be a decision, not a side effect. 34 -> 33 when
	// `session-tools:sessions-catch-up` became model-invocable so `maestro:your-cue`
	// can route to it itself. Update the number *and* regenerate
	// docs/codex/explicit-only-invocation-policy.md when this changes.
