...
Follow-ups behave exactly as local ones do, with one guard: the session cache records **which backend and which box** a host handle belongs to. `surface_ref` is an opaque string, so a herdr agent name from another machine is indistinguishable from a local one — re-dialing the workspace *without* `--remote` therefore starts a fresh callee and says so in `.fallbacks`, rather than handing the local herdr a name it has never heard of and stranding the real conversation on the other box.
