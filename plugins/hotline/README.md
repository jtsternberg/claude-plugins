...
It requires herdr on `PATH` and a reachable server, and it is **local only**. Placement and mode are open: a herdr callee is always a pane split off the caller's own, which *is* side-by-side, so `--placement side` and `--detached` are the same launch and differ only in the word `.placement` reports (`side` when the dial named none, exactly as over cmux). `--mode conference` works too — same launch, plus a focus once the prompt has landed (`herdr agent focus <name>`), and `.awaiting_response: false` because the session is the user's now. Two things are still refused: `--placement window` / `--window <ref>`, because hotline creates no herdr workspaces or tabs and so has no window to place a callee in; and `--remote`, which needs a remote transcript reader, because every hotline answer is read from the local `~/.claude/projects` tree (Phase 3). `--resume <someone-else's-session>` is refused for a different reason — herdr hosts a callee it *starts*, with the `--session-id` preset the transcript path depends on, and `claude --resume` cannot take that preset.
