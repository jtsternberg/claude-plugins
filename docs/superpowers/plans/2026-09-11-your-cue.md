...
**Spec:** `docs/superpowers/specs/2026-09-11-your-cue.md`
...
- Suggest burial only for completed, Graveyard-buryable sessions or wholly completed plots.
...
### Task 2: Add the Maestro Your Cue Skill

**Files:**
- Create: `plugins/maestro/skills/your-cue/SKILL.md`
- Create: `plugins/maestro/skills/your-cue/agents/openai.yaml`
- Modify: `plugins/maestro/README.md`
- Test: `plugins/maestro/tests/your-cue-skill_test.sh`
...
digest bounds, one cue per workstream, burial eligibility, final cue prioritization, and
the single zero-argument invocation.
...
workstream is already working or settled. Offer `bury this session` only when the
transcript establishes completion and Graveyard reports `buryable:true`. Offer `bury this
plot` only when all targetable sessions sharing that workspace are completed and
buryable. Keep burial as text in the briefing; execution requires a later explicit request.

- [ ] **Step 6: Verify**

```bash
bash plugins/maestro/tests/your-cue-skill_test.sh
```

Invoke `validate-dual-harness-skill` for `plugins/maestro/skills/your-cue`. Perform one
read-only run with both providers reachable and one that confirms before partial coverage.

- [ ] **Step 7: Commit**

```bash
git add plugins/maestro/skills/your-cue plugins/maestro/README.md plugins/maestro/tests/your-cue-skill_test.sh
git commit -m "add your cue briefing"
```
...
`update your cue plugin catalogs`, then invoke `publish-release` and verify its merge,
