...
#### 📖 [fable](plugins/fable)
Skills for working with — and like — the Fable model (`claude-fable-5`). Ships `fable-delegate`: delegation discipline that keeps a Fable main agent in the thinker/boss role — planning, judgment, and reviewing output — while routing file edits, searches, tests, and other mechanical execution to Opus/Sonnet subagents. And `think-like-fable`: the operating stance behind Fable's judgment — own the outcome, not the response — for agents running on other models, with the habits that stance generates (fix the class not the instance, follow your own diagnosis, spend the user's attention like a budget, verify end-to-end).
